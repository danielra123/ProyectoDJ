...
export class ComputerService {
  ...
  // Genera el ID del dispositivo
  const deviceId = Helper.generateDeviceId();

  let photoURL: URL | undefined;

  // Solo intenta guardar la foto si fue enviada
  if (request.photo) {
    try {
      photoURL = await this.photoRepository.savePhoto(request.photo, deviceId);
    } catch (error) {
      console.error("⚠️ Error al guardar la foto:", error);
      // Si falla el guardado, sigue sin foto
      photoURL = undefined;
    }
  }

  // Mapea la solicitud al dominio Computer
